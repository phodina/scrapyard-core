...
impl <'a>Pins<'a> {
	...
	pub fn new() -> Self {
		...
		let pin0 = Pin::IO{ name: "PA0".to_string(), position: Position::Linear(1),
    					params: Box::new(IOPin{	reset : true,
    											label : "Hello".to_string(),
    											signals: vec!["Input".to_string(), "Output".to_string(), "EXTI".to_string()],
    											current: signals[0]})};

		let pin1 = Pin::IO{ name: "PA1".to_string(), position: Position::Linear(2),
    					params: Box::new(IOPin{	reset : true,
    											label : "".to_string(),
    											signals: vec!["Input".to_string(), "Output".to_string(), "EXTI".to_string()],
    											current: Some(0)})};

    	let pin2 = Pin::IO{ name: "PA2".to_string(), position: Position::Linear(3),
    					params: Box::new(IOPin{	reset : true,
    											label : "".to_string(),
    											signals: vec!["Input".to_string(), "Output".to_string(), "EXTI".to_string()],
    											current: Some(0)})};

    	let pin3 = Pin::IO{ name: "PA3".to_string(), position: Position::Linear(4),
    					params: Box::new(IOPin{	reset : true,
    											label : "".to_string(),
    											signals: vec!["Input".to_string(), "Output".to_string(), "EXTI".to_string()],
    											current: Some(0)})};

    	let pin4 = Pin::IO{ name: "PA4".to_string(), position: Position::Linear(5),
    					params: Box::new(IOPin{	reset : true,
    											label : "".to_string(),
    											signals: vec!["Input".to_string(), "Output".to_string(), "EXTI".to_string()],
    											current: Some(0)})};

    	Pins{ pins: vec![pin0, pin1, pin2, pin3, pin4]}
		}
